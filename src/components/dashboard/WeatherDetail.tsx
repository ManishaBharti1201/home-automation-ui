...
const WeatherDetail: React.FC<WeatherDetailProps> = ({ onLog }) => {
  ...
  const now = new Date();
  const nextHourIndex = hourly.time.findIndex((t: string) => new Date(t) > now);
  const startIndex = nextHourIndex === -1 ? Math.max(0, hourly.time.length - 24) : nextHourIndex;

  const weatherCode =
    current.weathercode?.toString() || daily.weather_code[0]?.toString() || "0";
  const weatherInfo = weatherCodeMapping[weatherCode] || {
    description: "Unknown",
    image: "",
  };

  const isThunderstormWarning =
      ...
      {isThunderstormWarning && (
        <div className="absolute top-0 left-0 right-0 bg-red-600/80 backdrop-blur-sm text-white text-center py-3 rounded-t-[2.5rem] z-10 flex items-center justify-center gap-3">
          ...
          <span className="font-bold uppercase tracking-wide">
            Severe Weather Warning: Thunderstorms Expected!
          </span>
          <span className="text-xl animate-pulse">⚠️</span>
        </div>
      )}

      <div className={`${isThunderstormWarning ? "pt-16" : ""}`}>
        {" "}
        {/* Adjust padding if warning is present */}
          ...
          <div className="flex items-center gap-8">
            <div className="w-32 h-32 bg-white/70 bg-gradient-to-br from-cyan-500/20 to-transparent rounded-[2.5rem] border border-white/10 flex items-center justify-center p-4">
              <img
                ...
                alt="current"
                className="w-full h-full object-contain drop-shadow-[0_0_20px_rgba(34,211,238,0.4)]"
              />
            </div>
            <div>
              ...
              <div className="flex flex-col">
                ...
                <p className="text-[15px] font-bold text-white/60 uppercase tracking-[0.2em] mt-1">
                  Feels like{" "}
                  {Math.round(
                    hourly.apparent_temperature[0] || current.temperature,
                  )}
                  °C
                </p>
              </div>
          ...
          <div className="bg-white/5 border border-white/10 p-6 rounded-[2rem] flex flex-col justify-between min-h-[120px]">
            ...
            <div className="h-1.5 w-full bg-white/10 rounded-full mt-4 overflow-hidden">
              <div
                ...
                style={{
                  width: `${hourly.precipitation_probability[0] || daily.precipitation_probability_max[0]}%`,
                }}
              />
            </div>
          </div>
          <div className="bg-white/5 border border-white/10 p-6 rounded-[2rem] flex flex-col justify-between min-h-[120px]">
            ...
            <div className="h-1.5 w-full bg-white/10 rounded-full mt-4 overflow-hidden">
              <div
                ...
                style={{ width: `${hourly.relative_humidity_2m[0]}%` }}
              />
            </div>
          </div>
          <div className="bg-white/5 border border-white/10 p-6 rounded-[2rem] flex flex-col justify-between min-h-[120px]">
          ...
          <div className="flex overflow-x-auto pb-4 space-x-4 no-scrollbar">
            {hourly.time
              ...
              .map((time: string, i: number) => {
                ...
                const isSignificantPrecipitation =
                  rainAmount > 0.1 || snowfallAmount > 0.1;
                ...
                return (
                  <div
                    key={time}
                    className="flex-none w-32 p-4 rounded-2xl bg-slate-800/40 border border-white/10 flex flex-col items-center text-center group hover:bg-slate-700/60 hover:border-cyan-400/30 transition-all duration-300"
                  >
